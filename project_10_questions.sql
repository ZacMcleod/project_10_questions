...
#SELECT COUNT(*) AS row_count FROM final_airbnb
...
#select distinct neighborhood from final_airbnb
...
#select id, room_type, reviews_per_month from final_airbnb
...
#select distinct neighbourhood, COUNT(*) as counted_neighbourhoods from final_airbnb
#group by neighbourhood
#order by counted_neighbourhoods desc;

#select neighbourhood, avg(reviews_per_month) as avg_reviews_per_month from final_airbnb
#where neighbourhood = "Williamsburg"
#group by neighbourhood;
### The average reviews per month cut between Williamsburg listings is about .60

#select neighbourhood, avg(reviews_per_month) as avg_reviews_per_month from final_airbnb
#group by neighbourhood
#order by avg_reviews_per_month desc;
### The average reviews per month cut between Long Island City listings is about 2.23

### The conclusion is that Long Island city seems to be the most popular neighborhood because it has the most reviews per month per listing. 
### However, Williamsburg still is the most popular neighborhood because it has the most reviews per month.
### The amount of reviews per month per listing is not the question. What makes a neighborhood popular is how many sheer reviews it has.
### I think
...
#select id, neighbourhood, avg(reviews_per_month) as avg_reviews_per_month from final_airbnb
...
#select distinct host_name, count(host_name) as counted_host_names from final_airbnb
#group by host_name
#order by counted_host_names desc
#limit 1;
